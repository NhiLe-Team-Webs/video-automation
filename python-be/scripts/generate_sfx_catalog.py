...
PUBLIC_SFX_DIR = REPO_ROOT / "remotion-app" / "public" / "sfx"
...
def discover_sfx() -> list[str]:
    if not PUBLIC_SFX_DIR.exists():
        raise SystemExit(f"SFX directory not found: {PUBLIC_SFX_DIR}")

    assets: list[str] = []
    for path in sorted(PUBLIC_SFX_DIR.rglob("*")):
        if not path.is_file():
            continue
        relative = path.relative_to(PUBLIC_SFX_DIR).as_posix()
